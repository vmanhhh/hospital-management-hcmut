...
router.patch('/:id', getMedicine, async (req, res) => {
  ...
  if (req.body.dateImported != null) {
    res.medicine.dateImported = new Date(req.body.dateImported);
  }
  if (req.body.expirationDate != null) {
    res.medicine.expirationDate = new Date(req.body.expirationDate);
  }
  try {
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'Deleted Medicine' });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
});
